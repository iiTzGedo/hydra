...
export default function DocsPage() {
  ...
  const docIdFromPath = location.pathname.startsWith('/docs/')
    ? location.pathname.replace('/docs/', '').split('/')[0] || null
    : null;
                ...
                {searchResults.isLoading && searchQuery.trim().length > 0 ? (
                  ...
                  resultCards.map((entry) => (
                    <button
                      key={entry.docId}
                      type="button"
                      onClick={() => navigate(`/docs/${entry.docId}`)}
