        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "serviceId": {
                        "type": "string",
                        "description": "The service ID (e.g., svc::docker::nginx)",
                    },
                },
                "required": ["serviceId"],
            },
...
async def _execute_tool(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "list_nodes":
        ...
        # Parse service ID to get node ID
        parts = args["serviceId"].split("::")
        if len(parts) < 3:
            raise ValueError("Invalid service ID format")
        node_id = parts[-1]  # This is simplified; real impl would look up the node
