...
pub fn execute(args: &ServiceArgs, config_path: &PathBuf) -> Result<()> {
    ...
    match &args.command {
        ...
        Some(ServiceCommand::Run { once: _ }) => {
            ...
            Err(anyhow!(
                "Use 'hydra-agent run' instead of 'hydra-agent service run'"
            ))
        }
    }
}
...
fn activate_service(
    ...
    match runtime {
        ...
        ServiceRuntime::Cron => {
            let expr =
                cron_expr.ok_or_else(|| anyhow!("Cron expression is required for cron mode"))?;
...
fn start_docker_service() -> Result<()> {
    ...
    if !compose_path.exists() {
        return Err(anyhow!(
            "Docker compose file not found. Run 'service activate --docker' first."
        ));
    }
...
fn install_systemd_unit(install_dir: &PathBuf, config_path: &PathBuf) -> Result<()> {
    ...
    let config_dir = config_path
        .parent()
        .unwrap_or(std::path::Path::new("/etc/hydra"));
...
fn show_logs(lines: u32, follow: bool) -> Result<()> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match status {
            ...
            _ => {
                ...
                if log_dir.exists() {
                    println!(
                        "Event log not available. Check log files in: {}",
                        log_dir.display()
                    );
                } else {
                    println!(
                        "No logs available. Service may not have generated any log entries yet."
                    );
                }
            }
        }
    ...
    {
        return Err(anyhow!(
            "Log viewing is only supported on Linux and Windows"
        ));
    }
