...
use hydra_agent::config::{
    AgentConfig, ApiConfig, CollectionConfig, NodeConfig, ScheduleConfig,
};
...
fn seed_vault_with_api_key(vault: &Vault) {
    ...
    vault.save_api_key(&api_key).expect("Failed to seed API key");
}
...
async fn test_submit_profile_fails_after_all_retries() {
    ...
    assert!(result.unwrap_err().to_string().contains("Server unavailable"));
}
...
async fn test_report_event_skips_when_no_api_key() {
    ...
    let result = client
        .report_event("test", "Test", "No key", None)
        .await;
...
async fn test_register_with_credentials_login_fails() {
    ...
    let result = client
        ...
        .await;
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("INVALID_CREDENTIALS"));
}
